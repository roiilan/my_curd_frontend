...
const KEY_FAVORS = 'products'
const KEY_PROJS_COUNT = 'productsCount'
...
function changeProductsCount(diff) {
   var productsCount = storageService.load(KEY_PROJS_COUNT)
   if (productsCount) {
       productsCount += diff
       storageService.store(KEY_PROJS_COUNT, productsCount)
   }
}
...
async function getProductsCount(){
    ...
    if (!productsCount) {   
       ...
       productsCount = await httpService.get(`product?${queryParams}`);
       storageService.store(KEY_PROJS_COUNT, productsCount)
   } 
...
function query(filterBy, limit, skip) {
    ...
    if (filterBy) {
        ...
        return httpService.get(`product?${queryParams}`);
    }
    if (limit) {
        queryParams.set('limit', limit)
        return httpService.get(`product?${queryParams}`);
...
function save(product) {
    return (product._id) ? _update(product) : _add(product);
}
...
function loadCategoties() {
    return [
        ...
        },
        {
            ...
            title: "Art",
            imgUrl:
                "https://levlalev.com/wp-content/uploads/2019/06/Art-Productec-to-benefit-orphans.jpg"
        },
        {
...
function getHeaderObj(filter) {
    ...
    var headerObj = {
        'animalsAndWildlife': {
            ...
            desc: 'Wildlife conservation is the perfect volunteer opportunity to protect endangered species from extinction. Volunteer for wildlife conservation productects and help endangered species, like elephants or sloths, that are threatened by habitat loss and human influences. Care for exotic or domestic wildlife and prevent species from going extinct!',
        ...
        'environmentalProtection': {
            title: 'Environmental Protection',
            desc: 'Environmental volunteering abroad is your chance to protect our nature on land as well as below the water. If you love the great outdoors and hands-on work, then our nature conservancy productects will be the perfect choice for you!',
        ...
        'art': {
            title: 'Art',
            desc: 'Volunteer for art productects and use all your creativity to do something meaningful whilst you travel the world. Whether you play an instrument, are gifted in arts and crafts or have a passion for photography, there are many productects worldwide focusing on these topics.',
        ...
        'humanRights': {
            title: 'HumanRights',
            desc: 'Volunteer for women\'s empowerment productects and help to give a voice to those who are otherwise unheard. Gender equality is a big topic in the western world and more important than ever. Sometimes we forget that there are countries where women\'s rights are still pretty much non-existent.',
